...
import { AssetsApiError, probeAssetListAccess } from './api/physical-assets-api';
...
export function AssetsRoute({ children }: AssetsRouteProps) {
  ...
  useEffect(() => {
    ...
    void probeAssetListAccess(controller.signal)
      .then((allowed) => {
        if (!cancelled) {
  ...
  if (state === 'denied') {
    return (
      <Navigate
        to="/app/no-access"
        replace
        state={{ from: location.pathname, capabilityId: 'resources:asset:list' }}
      />
    );
  }
