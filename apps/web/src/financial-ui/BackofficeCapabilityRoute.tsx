...
export function BackofficeCapabilityRoute({
  children,
  probe,
  capabilityId,
}: {
  ...
  }, [expireSession, location.pathname, probe]);
  ...
  if (state === 'denied') {
    return (
      <Navigate to="/app/no-access" replace state={{ from: location.pathname, capabilityId }} />
    );
  }
