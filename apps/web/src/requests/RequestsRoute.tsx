...
import { probeServiceRequestListAccess, ServiceRequestsApiError } from './api/service-requests-api';
...
export function RequestsRoute({ children }: RequestsRouteProps) {
  ...
  useEffect(() => {
    ...
    void probeServiceRequestListAccess(controller.signal)
      .then((allowed) => {
        if (!cancelled) {
      ...
      .catch((error: unknown) => {
        ...
        if (error instanceof ServiceRequestsApiError && error.status === 401) {
          expireSession();
          setState('session_expired');
          return;
        }
        if (error instanceof ServiceRequestsApiError && error.status === 403) {
          setState('denied');
          return;
        }
        setState('error');
  ...
  if (state === 'denied') {
    return (
      <Navigate
        to="/app/no-access"
        replace
        state={{ from: location.pathname, capabilityId: 'requests:service-request:list' }}
      />
    );
  }
