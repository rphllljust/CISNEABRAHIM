...
import { CatalogApiError, probeCatalogListAccess } from './api/service-catalog-api';
...
export function CatalogRoute({ children }: CatalogRouteProps) {
  ...
  useEffect(() => {
    ...
    void probeCatalogListAccess(controller.signal)
      .then((allowed) => {
        if (!cancelled) {
  ...
  if (state === 'denied') {
    return (
      <Navigate
        to="/app/no-access"
        replace
        state={{ from: location.pathname, capabilityId: 'catalog:service:list' }}
      />
    );
  }
