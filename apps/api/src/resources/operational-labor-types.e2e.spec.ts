...
describe('Operational labor types E2E', () => {
  ...
  beforeAll(async () => {
    if (!testDatabaseUrl) {
      throw new Error('TEST_DATABASE_URL is required for operational labor types E2E tests.');
    }
  ...
  async function loginWithLaborTypeGrants(): Promise<{ accessToken: string }> {
    const loginId = normalizeLoginIdentifier(`labor-e2e-${crypto.randomUUID()}@cisne.invalid`);
    ...
    const response = await app.inject({
      ...
      headers: { 'user-agent': 'vitest-labor-e2e' },
    });
    const body = parseAuthTokenResponse(response.body);
    return { accessToken: body.accessToken };
  }

  it('lists canonical labor types via HTTP', async () => {
    const { accessToken } = await loginWithLaborTypeGrants();
    const response = await app.inject({
      method: 'GET',
      url: '/api/v1/resources/labor-types?limit=50&offset=0',
      headers: { authorization: `Bearer ${accessToken}` },
    });
    expect(response.statusCode).toBe(200);
    const body = JSON.parse(response.body) as { items: Array<{ code: string }> };
    expect(body.items.map((item) => item.code)).toContain('DRIVER');
  });
});
