...
describe('Units of measure E2E', () => {
  ...
  beforeAll(async () => {
    if (!testDatabaseUrl) {
      throw new Error('TEST_DATABASE_URL is required for units of measure E2E tests.');
    }
  ...
  async function loginWithUnitGrants(): Promise<{ accessToken: string }> {
    const loginId = normalizeLoginIdentifier(`units-e2e-${crypto.randomUUID()}@cisne.invalid`);
    ...
    const response = await app.inject({
      ...
      headers: { 'user-agent': 'vitest-units-e2e' },
    });
    const body = parseAuthTokenResponse(response.body);
    return { accessToken: body.accessToken };
  }

  it('lists canonical units via HTTP', async () => {
    const { accessToken } = await loginWithUnitGrants();
    const response = await app.inject({
      method: 'GET',
      url: '/api/v1/catalog/units-of-measure?limit=50&offset=0',
      headers: { authorization: `Bearer ${accessToken}` },
    });
    expect(response.statusCode).toBe(200);
    const body = JSON.parse(response.body) as { items: Array<{ code: string }> };
    expect(body.items.map((item) => item.code)).toContain('M3');
  });
});
