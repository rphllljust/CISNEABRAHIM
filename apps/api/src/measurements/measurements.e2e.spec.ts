...
describe('Measurements E2E', () => {
  ...
  beforeAll(async () => {
    if (!testDatabaseUrl) {
      throw new Error('TEST_DATABASE_URL is required for measurements E2E tests.');
    }
  ...
  beforeEach(async () => {
    ...
    const login = normalizeLoginIdentifier(`msr-e2e-${crypto.randomUUID()}@cisne.invalid`);
  ...
  it('denies measurement creation without measurement:create grant', async () => {
    ...
    const measurementResponse = await app.inject({
      method: 'POST',
      url: `/api/v1/service-orders/${created.id}/measurements`,
      headers: { authorization: `Bearer ${accessToken}` },
    });
  ...
  it('rejects measurement creation when service order is not completed', async () => {
    const login = normalizeLoginIdentifier(`msr-e2e-admin-${crypto.randomUUID()}@cisne.invalid`);
    const passwordHash = await hashPassword(AUTH_TEST_PASSWORD);
    const { identityId } = await insertIdentity(pool, login, passwordHash);
    for (const action of [
      AUTHZ_ACTIONS.ServiceOrdersServiceOrderCreate,
