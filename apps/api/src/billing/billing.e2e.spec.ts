...
describe('Billing E2E', () => {
  ...
  beforeAll(async () => {
    if (!testDatabaseUrl) {
      throw new Error('TEST_DATABASE_URL is required for billing E2E tests.');
    }
  ...
  beforeEach(async () => {
    ...
    const login = normalizeLoginIdentifier(`bil-e2e-${crypto.randomUUID()}@cisne.invalid`);
  ...
  it('returns 403 when billing prepare is denied', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: `/api/v1/service-orders/${serviceOrder.id}/billing-records`,
      headers: { authorization: `Bearer ${accessToken}` },
      payload: {
